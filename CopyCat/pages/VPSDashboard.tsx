...
const VPSDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${API_BASE_URL}/vps-dashboard?month=${selectedMonth}`);
        if (!response.ok) {
  ...
  const iconMap = {
    "Çalışan Ortalaması": Users,
    "Aktif Çalışan Ortalaması": Activity,
    "Puantaj Günü": Calendar,
    "Tabak Sayısı": Target,
    "VPS": BarChart3,
  };

  const colorMap = {
      "Çalışan Ortalaması": "from-blue-500 to-blue-600",
      "Aktif Çalışan Ortalaması": "from-green-500 to-green-600",
      "Puantaj Günü": "from-purple-500 to-purple-600",
      "Tabak Sayısı": "from-orange-500 to-orange-600",
      "VPS": "from-indigo-500 to-indigo-600",
  };

  const scoreDataClassNameMap = {
    "Bayram Arife": "bg-gradient-to-r from-blue-50 to-blue-100 text-blue-800 border-blue-200",
    "Bayram Tatil": "bg-gradient-to-r from-emerald-50 to-emerald-100 text-emerald-800 border-emerald-200",
    "Çalışıyor": "bg-gradient-to-r from-amber-50 to-amber-100 text-amber-800 border-amber-200",
    "Çıkış": "bg-gradient-to-r from-red-50 to-red-100 text-red-800 border-red-200",
    "Haftalık İzin": "bg-gradient-to-r from-violet-50 to-violet-100 text-violet-800 border-violet-200",
    "İzinde Çalışma": "bg-gradient-to-r from-teal-50 to-teal-100 text-teal-800 border-teal-200",
    "Yıllık İzin": "bg-gradient-to-r from-pink-50 to-pink-100 text-pink-800 border-pink-200",
  };
  ...
  const { mainData, scoreData, dates, iseGirenCalisanSayisi, istenCikanCalisanSayisi } = data;
          ...
          {mainData.slice(0, 2).map((stat, index) => {
            ...
            <div key={index} className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-white/50">
              <div className="flex items-center justify-between mb-4">
                <div className={`p-3 rounded-xl bg-gradient-to-r ${colorMap[stat.label]} shadow-lg`}>
                  ...
                  <tr key={index} className="border-b border-slate-200/50 hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-transparent transition-all duration-300 group">
                    <td className="p-4 font-bold text-slate-700 border-r-2 border-slate-200 bg-gradient-to-r from-slate-50/80 to-white/80 sticky left-0 z-10 group-hover:from-blue-100/80 group-hover:to-blue-50/80 transition-all duration-300">
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg bg-gradient-to-r ${colorMap[row.label]} shadow-md`}>
                ...
                {scoreData.map((row, index) => (
                  <tr key={`score-${index}`} className="border-b border-slate-200/50 hover:bg-gradient-to-r hover:from-slate-50/50 hover:to-transparent transition-all duration-300 group">
                    <td className="p-4 border-r-2 border-slate-200 bg-gradient-to-r from-slate-50/80 to-white/80 sticky left-0 z-10 group-hover:from-slate-100/80 group-hover:to-slate-50/80 transition-all duration-300">
                      <div className="flex items-center justify-between">
                        <div className="font-bold text-slate-800">{row.label}</div>
                        <div className={`px-3 py-1 rounded-full text-xs font-bold border-2 ${scoreDataClassNameMap[row.label]}`}>
                          {row.multiplier}
                        </div>
                      </div>
                    </td>
